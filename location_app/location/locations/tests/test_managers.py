...
class TestLocationManager(TestCase):
    def setUp(self):
        self.warsaw = LocationFactory(name="Warsaw", location=Point(21.003778, 52.212667))
        self.oslo = LocationFactory(name="Oslo", location=Point(10.783329, 59.916950))
        self.kielce = LocationFactory(name="Kielce", location=Point(20.645882, 50.862886))
    ...
    def test_check_if_exists_return__true__when_location_exists(self):
        check = Location.objects.check_if_exists(location=Point(self.oslo.location.x, self.oslo.location.y))
    ...
    def test_find_closest_return_queryset(self):
        closest_location = Location.objects.find_closest_queryset(self.warsaw.location.x, self.warsaw.location.y)
        self.assertTrue(isinstance(closest_location, QuerySet))

        closest_locations = Location.objects.find_closest_queryset(self.warsaw.location.x, self.warsaw.location.y, self.distance)
        self.assertTrue(isinstance(closest_locations, QuerySet))

    def test_find_closest_place(self):

        closest_location = Location.objects.find_closest_queryset(self.point_piaseczno.x, self.point_piaseczno.y).all()
    ...
    def test_find_closest_places(self):
        closest_locations = Location.objects.find_closest_queryset(self.point_piaseczno.x, self.point_piaseczno.y, self.distance).all()
