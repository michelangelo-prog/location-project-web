...
def mocked_requests_get(*args, **kwargs):
    ...
    if url == AdapterElevationAPI.API_URL and params == {
        "points": "{0},{1}".format(TEST_LATITUDE, TEST_LONGITUDE)
    }:
        return MockResponse(
            {
                "elevations": [
    ...
    elif url == AdapterElevationAPI.API_URL and params == {
        "points": "{0},{1}".format(TEST_LONGITUDE_WRONG, TEST_LATITUDE_WRONG)
    }:
        return MockResponse(
            {
                "elevations": [{"lat": TEST_LATITUDE, "lon": TEST_LONGITUDE}],
                "resolution": "5000m",
            },
            HTTP_200_OK,
        )
...
class TestAdapterElevationAPI(TestCase):
    ...
    @mock.patch(
        "location.locations.adapters.requests.get", side_effect=mocked_requests_get,
    )
    ...
    def test_raise_ElevationApiException_when_status_code_different_than_200(
        self, mock_get
    ):
    ...
    def test_raise_ElevationApiException_when_json_data_different_than_200(
        self, mock_get
    ):
        with self.assertRaises(ElevationApiException):
            self.elevation_adapter.get_elevation(
                TEST_LATITUDE_WRONG, TEST_LONGITUDE_WRONG
            )
